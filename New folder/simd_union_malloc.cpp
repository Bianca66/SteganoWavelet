...
struct BMP
{
	...
	void readBMP(const char* filename)
	{
		...
		data_simd = (union  simd_union*)malloc(h * w * sizeof(union simd_union));

		int i = 0;
		for (int y = 0; y < h; y++) {
			for (int x = 0, i = 0; x < w; x++, i+=4) {
				data_simd[i].a_simd = _mm_setr_ps((float)data[ch * (y * w + x) + 0], (float)data[ch * (y * w + x) + 1], (float)data[ch * (y * w + x) + 2], 0);
			}
		}
	...
	void writeBMP(const char* fname)
	{
		for (int y = 0; y < h; y++) {
			for (int x = 0, i = 0; x < w; x++, i+=4) {
				data[ch * (y * w + x) + 0] = data_simd[i].a[0];
				data[ch * (y * w + x) + 1] = data_simd[i].a[1];
				data[ch * (y * w + x) + 2] = data_simd[i].a[2];
			}
		}
