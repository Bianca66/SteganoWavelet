...
struct BMP
{
	...
	void readBMP(const char* filename)
	{
		...
		a = (float*)_mm_malloc(sizeof(float) * w * h * ch, 32); // mm malloc aligns to 32 bytes
		__m128* data_simd = (__m128*) a;

		int i = 0;
		for (int y = 0; y < h; y++) {
			for (int x = 0, i = 0; x < w; x++, i += 4) {
				*(data_simd + i) = _mm_setr_ps((float)data[ch * (y * w + x) + 0], (float)data[ch * (y * w + x) + 1], (float)data[ch * (y * w + x) + 2], 0);
			}
		}
	...
	void writeBMP(const char* fname)
	{
		for (int y = 0; y < h; y++) {
			for (int x = 0, i = 0; x < w; x++, i += 4) {
				data[ch * (y * w + x) + 0] = a[i];
				data[ch * (y * w + x) + 1] = a[i + 1];
				data[ch * (y * w + x) + 2] = a[i + 2];
			}
		}
